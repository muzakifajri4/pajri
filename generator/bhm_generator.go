...
// GenerateBHM creates the full BHM report:
// REKAPITULASI REALISASI BELANJA DANA BOSP ( BARANG MODAL / ASET )
func GenerateBHM(path string, records []parser.BHMRecord, header parser.BHMHeaderInfo) error {
	f := excelize.NewFile()
	defer f.Close()
	...
	f.SetCellValue("Sheet1", titleCell, "REKAPITULASI REALISASI BELANJA DANA BOSP ( BARANG MODAL / ASET )")
	...
	// Header row
	row = 6
	headers := []string{"Tanggal",
		"Kode\nKegiatan", "Kode Rekening", "No. Bukti", "ID Barang",
		"Uraian", "Jumlah\nBarang", "Harga\nSatuan", "Realisasi"}
...
// Total row
	...
	footerText := fmt.Sprintf("BHM %s %s - NPSN : %s, Nama Sekolah : %s", header.Month, header.Year, header.NPSN, header.NamaSekolah)
	...
	if err := f.SaveAs(path); err != nil {
		return fmt.Errorf("failed to save BHM Excel: %w", err)
	}

	return nil
}
// GenerateBHMImport creates the import file matching the
// "Template-Import-BM-ARKAS" template (8 columns, sheet name "Sheet2").
func GenerateBHMImport(path string, records []parser.BHMRecord, header parser.BHMHeaderInfo) error {
	f := excelize.NewFile()
	defer f.Close()

	// Rename default sheet to match the template
	...
	// Set column widths to mirror the template
	f.SetColWidth("Sheet2", "A", "A", 14)
	f.SetColWidth("Sheet2", "B", "B", 16)
	f.SetColWidth("Sheet2", "C", "C", 18)
	f.SetColWidth("Sheet2", "D", "D", 12)
	f.SetColWidth("Sheet2", "E", "E", 22)
	f.SetColWidth("Sheet2", "F", "F", 45)
	f.SetColWidth("Sheet2", "G", "G", 15)
	f.SetColWidth("Sheet2", "H", "H", 15)

	// Header row (matches Template-Import-BM-ARKAS)
	row := 1
	headers := []string{"TANGGAL", "KODE KEGIATAN", "KODE REKENING", "NO. BUKTI", "ID BARANG", "URAIAN", "JUMLAH BARANG", "HARGA SATUAN"}
	for i, h := range headers {
		col := string(rune('A' + i))
		cell := fmt.Sprintf("%s%d", col, row)
	...
	for _, record := range records {
		...
		f.SetCellValue("Sheet2", colE, record.IDBarang)
		f.SetCellStyle("Sheet2", colE, colE, dataStyle)

		f.SetCellValue("Sheet2", colF, record.Uraian)
		f.SetCellStyle("Sheet2", colF, colF, dataStyle)

		f.SetCellValue("Sheet2", colG, record.JumlahBarang)
		f.SetCellStyle("Sheet2", colG, colG, numberStyle)

		f.SetCellValue("Sheet2", colH, record.HargaSatuan)
		f.SetCellStyle("Sheet2", colH, colH, numberStyle)
	...
	// Save
	if err := f.SaveAs(path); err != nil {
		return fmt.Errorf("failed to save BHM Template Import Excel: %w", err)
	}
